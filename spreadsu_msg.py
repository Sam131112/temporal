...
top5b = []
unodes = []
sp  = open("original_spread_result_su.txt","w")
sp1 = open("original_bet_result_su.txt","w")

for v in range(1,70,1):
	...
	c1 = pickle.load(open("../super_results1/superu_"+str(v)+"close.p","rb"))
        c2 = c1.items()
	...
	output = os.popen('java MultiSpread '+str(size)+" "+"temps.txt").read()
	out = os.popen('./new_centrality '+'seed_betw.txt '+"temps.txt").read()
	op = output.split()
	print out
	print op
